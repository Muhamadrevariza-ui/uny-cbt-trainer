...
import { ChevronLeft, ChevronRight, CheckCircle, GripHorizontal, X } from 'lucide-react';
...
export default function Exam() {
  ...
  useEffect(() => {
    if (!exam) return;
    const timeRemainingSec = Math.max(0, exam.durationSec - Math.floor((now - exam.startedAt) / 1000));
  ...
  const timeRemainingSec = Math.max(0, exam.durationSec - Math.floor((now - exam.startedAt) / 1000));
  const isTimeLow = timeRemainingSec < 60 && timeRemainingSec > 0;
    ...
    <div className="min-h-[100dvh] bg-slate-50 flex flex-col relative select-none">
      <div className="bg-white border-b px-4 py-3 flex items-center justify-between sticky top-0 z-20 shadow-sm">
        ...
        <div className="text-center px-2">
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{SECTION_LABELS[question.section]}</div>
          <div className="font-bold text-slate-800 text-[15px] mt-0.5">Soal {qIndex + 1} / {exam.questions.length}</div>
        </div>
      ...
      {isDrawerOpen && (
        <div className="absolute inset-0 z-50 flex flex-col bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          <div className="bg-white rounded-t-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-full duration-300">
            ...
            <div className="grid grid-cols-5 gap-3 max-h-[50vh] overflow-y-auto pb-4 px-1 content-start">
              {exam.questions.map((q, i) => {
                ...
                if (ans !== null && !dbt) btnClass = "bg-primary text-white border-2 border-primary shadow-sm"; 
                if (ans !== null && dbt) btnClass = "bg-amber-400 text-white border-2 border-amber-500 shadow-sm"; 
                if (ans === null && dbt) btnClass = "bg-amber-50 text-amber-600 border-2 border-amber-300"; 
      ...
      {isSubmitOpen && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-5 animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] p-6 shadow-2xl w-full max-w-sm animate-in zoom-in-95 duration-200 relative overflow-hidden">
            ...
            <p className="text-center text-[13px] font-medium text-slate-500 mb-6 relative z-10">Waktu tersisa: <span className="font-mono font-bold">{fmtClock(timeRemainingSec)}</span></p>
